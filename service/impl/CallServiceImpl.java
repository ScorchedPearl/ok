...
//    @Value("${ai.calling.service}")
    ...
    public CallServiceImpl(
        ...
        this.webClient = webClientBuilder.baseUrl("https://api.screenera.ai/api/call").build();
...
//        apiRequestBody.put("status", call.getStatus().name());
        ...
        apiRequestBody.put("firstMessage", "Hello! This is Sarah from the hiring team. Thank you for participating in this interview. I’ll be asking a series of questions about your experience and skills, and you’ll have a chance to ask questions at the end. The interview will take about 15-20 minutes. Are you ready to begin?");
...
public void updateCallAndAddTranscripts(CallUpdateRequest request,String token) {
    ...
    Call call = callRepository.findById(request.getCallId())
            .orElseThrow(() -> new IllegalArgumentException("Call not found with ID: " + request.getCallId()));

    // Update the status of the Call
    call.setStatus(CallStatus.valueOf(request.getStatus().toUpperCase()));
    callRepository.save(call);

    // Clear existing transcripts if you want to replace them (optional)
    // transcriptRepository.deleteByCall(call);
    ...
    if (request.getTranscript() != null && !request.getTranscript().isEmpty()) {
        List<Transcript> transcripts = request.getTranscript().stream().map(data -> {
            ...
            transcript.setCallIdRef(data.getCallId()); // Store the callId from transcript
    ...
    public void analyzeTranscript(UUID callId,String token) {
        ...
        try {
            ...
            logger.info("Successfully analyzed and updated summary for call ID: {}", callId);
        } catch (Exception e) {
            logger.error("Failed to analyze transcript for call ID: {}", callId, e);
            throw new RuntimeException("Failed to analyze transcript: " + e.getMessage(), e);
        }
    }
