...
// For position, resume content, summary, and file URL, use provided values or copy from previous interview
                    ...
                    try {
                        ...
                        try {
                            ...
                            String emailContent = interviewerTemplate
                                    ...
                                    .replace("{resumeUrl}", dto.getResumeFileUrl() != null ? dto.getResumeFileUrl() : "N/A");
            ...
            logger.error("Error during createInterview process. Candidate ID: {}, Email: {}, Position: {}. Exception: {}",
                    dto.getCandidateEmail(), dto.getPosition(), e.getMessage(), e);
...
//        return interviewMapper.toDto(interview);
    ...
    public List<Map<String, Object>> getInterviewsByTenantId(Long tenantId) {
        log.info("Fetching interviews for tenant ID: {}", tenantId);
        try {
            List<Interview> interviews = interviewRepository.findByTenantId(tenantId);
            return interviews.stream()
                    .map(interview -> {
                        Map<String, Object> interviewMap = new HashMap<>();
                        interviewMap.put("interviewId", interview.getInterviewId());
            ...
            log.error("Error fetching interviews for tenant {}: {}", tenantId, e.getMessage());
            throw new RuntimeException("Failed to fetch interviews for tenant " + tenantId, e);
        }
    }
...
//        Interview interview = getInterviewById(interviewId);
    ...
    /**
     * Retrieves interview IDs for interviews where the specified user is an interviewer.
     *
     * @param userId the ID of the interviewer
     * @return a list of interview IDs
     */
    public List<Long> getInterviewIdsByInterviewerId(Long userId) {
        log.info("Fetching interview IDs for interviewer with userId: {}", userId);

        List<InterviewersInterview> interviews = interviewersInterviewRepository.findByIdUserId(userId);

        List<Long> interviewIds = interviews.stream()
                .map(interview -> interview.getInterview().getInterviewId())
                .collect(Collectors.toList());

        log.info("Found {} interviews for interviewer with userId: {}", interviewIds.size(), userId);
        return interviewIds;
    }


//    public List<InterviewResponseDto> getInterviewsByCandidateId(Long candidateId) {
//        List<Interview> interviews = interviewRepository.findByCandidateId(candidateId);
//        return interviews.stream()
//                .map(this::mapInterviewToResponseDto)
//                .collect(Collectors.toList());
//    }


    public List<InterviewResponseDto> getInterviewsByCandidateIdAndTenantId(Long candidateId, Long tenantId) {
        List<Interview> interviews = interviewRepository.findByCandidateIdAndTenantId(candidateId, tenantId);
        return interviews.stream()
                .map(this::mapInterviewToResponseDto)
                .collect(Collectors.toList());
    }
    private InterviewResponseDto mapInterviewToResponseDto(Interview interview) {
        return interviewMapper.toDto(interview);
    }
