...
public class DocumentParserController {
    ...
    public ResponseEntity<CandidateMatchScoreResponseDto> calculateMatchScore(@RequestBody CandidateMatchScoreRequestDto request) {
        log.info("Calculating match score between candidate resume and job description");
        try {
            if (request.getJobDescription() == null || request.getJobDescription().trim().isEmpty() ||
                    request.getResume() == null || request.getResume().trim().isEmpty()) {
                return ResponseEntity.badRequest().body(
                        CandidateMatchScoreResponseDto.builder()
            ...
            double matchPercentage = aiFeedbackService.calculateResumeJobMatchPercentage(
                    request.getJobDescription(), request.getResume());

//            // Retrieve match details from AI service
//            Map<String, String> matchDetails = aiFeedbackService.getMatchDetails(
//                    request.getJobDescription(), request.getResume());

            // Generate candidate fit summary using AI service
            String fitSummary = aiFeedbackService.generateCandidateFitSummary(
                    request.getJobDescription(), request.getResume(), matchPercentage);

            // Retrieve skills and years of experience from AI service
            String[] skills = aiFeedbackService.getMatchedSkills(
                    request.getJobDescription(), request.getResume());
            int yearsOfExperience = aiFeedbackService.getYearsOfExperience(
                    request.getJobDescription(), request.getResume());
...
//                            .matchDetails(matchDetails)
            ...
            log.error("Error calculating match score: {}", e.getMessage(), e);
            return ResponseEntity.badRequest().body(
                    CandidateMatchScoreResponseDto.builder()
                            .error("Failed to calculate match score: " + e.getMessage())
                            .build()
            );
        }
    }
