...
public class SwotAnalysisController {
    ...
    public ResponseEntity<SwotAnalysisResponseDto> generateSwotAnalysis(@RequestBody SwotAnalysisRequestDto request,@RequestHeader("Authorization") String token) {
        log.info("Received request to generate SWOT analysis for candidate: {}, position: {}",
                request.getCandidateName(), request.getPosition());

        SwotAnalysisResponseDto response = swotAnalysisService.generateSwotAnalysis(request,token);
